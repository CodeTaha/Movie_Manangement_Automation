    ...
    def bilgileri_guncelle(self):
        ...
        if yeni_film_adi:
            ...
            if film:
                ...
                print("Girmiş olduğunuz film bilgileri bulunamadı. Lütfen öncelikel filmi sisteme ekleyin daha sonrasında flm güncellemesi yapınız.")
...
class FilmKutuphanesi:
    ...
    def yonetmen_aramak(self, yonetmen_ad_soyad):
        row = self.cursor.execute('SELECT * FROM Yonetmen WHERE LOWER(ad_soyad) LIKE ?', (f'%{yonetmen_ad_soyad}%',)).fetchone()
        if row:
            return Yonetmen(row[1], row[2])
        return None

    def sifre_guncelle(self, kullanici_adi, yeni_sifre):
    ...
    def kullanici_guncelleme(self, kullanici_adi):
        row = self.cursor.execute('SELECT * FROM Kullanici WHERE LOWER(kullanici_adi) LIKE ?', (f'%{kullanici_adi}%',)).fetchone()
        if row:
            kullanici = Kullanici(row[1], row[2], row[3], row[4], row[5])
            ...
            ''', (kullanici.ad, kullanici.soyad, kullanici.email, kullanici.kullanici_adi, kullanici.sifre, f'%{kullanici_adi}%')).fetchall()
            self.connect.commit()
        else:
            print("Kullanıcı bulunamadı.")
    ...
    def yonetmen_guncelleme(self, yonetmen_ad_soyad):
        ...
        if row:
            ...
            self.cursor.execute('''
                ...
                WHERE ad_soyad = ?
            ''', (yonetmen.yonetmen_ad_soyad, yonetmen.dogum_yili, yonetmen_ad_soyad, f'%{yonetmen_ad_soyad}%')).fetchall()
            self.connect.commit()
        else:
    ...
    def film_guncelleme(self, film_adi):
        ...
        if row:
            ...
            self.cursor.execute('''
                ...
                WHERE film_adi = ?
            ''', (film.film_adi, film.yonetmen_ad_soyad, film.cikis_yili, film.tur, film_adi, f'%{film_adi}%')).fetchone()
            self.connect.commit()
        else:
...
while True:
    ...
    if islem == "1":
        ...
        film_kutuphanesi.sifre_guncelle(kullanici_adi, yeni_sifre)
        time.sleep(3)
